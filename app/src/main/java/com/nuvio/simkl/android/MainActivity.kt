...
class MainActivity : AppCompatActivity() {
    ...
    private fun buildUi() {
        ...
        port = field("Local port", c.port.toString())
        root.addView(port)

        simkl = field("Simkl Client ID", c.simklClientId)
        root.addView(simkl)

        tmdb = field("TMDB API key", c.tmdbApiKey)
        root.addView(tmdb)

        tvdb = field("TVDB API key", c.tvdbApiKey)
        root.addView(tvdb)

        discordApp = field(
            "Discord Application ID",
            if (c.discordApplicationId == 0L) {
                ""
            } else {
                c.discordApplicationId.toString()
            }
        )
        root.addView(discordApp)

        username = field("Nuvio username", c.profileUsername)
        root.addView(username)

        avatar = field("Nuvio avatar URL", c.profileAvatarUrl)
        root.addView(avatar)
        ...
        save.setOnClickListener {
            val cfg = AppConfig(
                ...
                discordApplicationId =
                    discordApp.text.toString().trim().toLongOrNull() ?: 0L,
            ...
            stopService(
                Intent(this, NuvioService::class.java)
            )
            ...
            status.text =
                "Running at http://127.0.0.1:${cfg.port}"
        }
